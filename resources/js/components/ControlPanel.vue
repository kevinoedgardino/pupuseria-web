      ...
      <v-data-table :headers="headers" :items="productos" class="elevation-1">
        <template v-slot:top>
          <v-container class="d-flex">
            <v-row>
              <v-col class="ventas container-fluid m-2 text-light rounded-3" cols="12" lg="6">
              ...
              <v-col cols="12" lg="6">
                <v-menu v-model="menu2" :close-on-content-click="false" transition="scale-transition" offset-y
                  ...
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field v-model="computedDateFormatted" label="Fecha de hoy" hint="Formato Dia/Mes/Año"
          ...
          <v-toolbar flat>
            ...
            <v-spacer></v-spacer>
            <v-dialog v-model="dialog" max-width="500px" style="background: white !important;">
              <template v-slot:activator="{ on, attrs }">
              ...
              <div style="background: #fff">
                    ...
                    <v-form ref="formProducto" v-model="validForm" :lazy-validation="true">
                      <v-row>
                        <v-col cols="12" sm="6" md="4">
                          <v-text-field required v-model="producto.nombre" label="Nombre"></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="6" md="4">
                          <v-select v-model="producto.tipoprod_id" :items="tipos" label="Tipo de producto"
                            item-value="id" item-text="nombre"></v-select>
                        </v-col>
                        <v-col cols="12" sm="6" md="4">
                          <v-text-field v-model="producto.precio" required label="Precio"></v-text-field>
                        </v-col>
                      </v-row>
                    </v-form>
                  </v-container>
                ...
                <v-card-actions>
                  ...
                  <v-btn color="blue darken-1" text @click="saveProducto()">
                    Guardar
                  </v-btn>
                </v-card-actions>
              </div>
        ...
        <template v-slot:item.actions="{ item }">
          <v-btn class="ma-2" color="indigo" dark @click="editProducto(item)">
            <v-icon dark>
              edit
            </v-icon>
          </v-btn>
          <v-btn class="ma-2" color="red" dark @click="deleteProducto(item)">
            <v-icon dark>
              remove_circle
            </v-icon>
          </v-btn>
        ...
        <v-card>
          <v-card-title>
            ...
            <div class="text-center ml-5">
              <v-btn color="blue-grey" class="ma-2 white--text" href="/pedidos/reporte">
                Generar PDF
                <v-icon right dark>
                  picture_as_pdf
                </v-icon>
              ...
              <v-dialog transition="dialog-bottom-transition" max-width="600">
                ...
                <template v-slot:default="dialog">
                  <v-card>
                    <v-toolbar color="primary" dark class="h3">Elige el rango de fechas para generar el reporte <v-icon
                        class="ml-2">event_available</v-icon>
                    </v-toolbar>
                    <v-card-text>
                      <p class="text-center mt-6 h5">Fecha desde el {{ dateRangeText }}</p>
                    </v-card-text>
                    <div class="d-flex justify-content-center">
                      <v-date-picker class="mt-4" v-model="dates" range>
                      </v-date-picker>
                    </div>
                    <v-card-actions class="justify-end">
                      <v-btn text class="border border-success" @click="generarPdf" color="green">Generar</v-btn>
                      <v-btn text class="border border-danger" @click="dialog.value = false; dates = []"
                        color="red">Cancelar</v-btn>
                    </v-card-actions>
                  </v-card>
                </template>
              </v-dialog>
              <v-badge :value="hover" color="green accent-5"
                content="Estados: E = Entregado, P = Pendiente, C = Cancelado" right transition="slide-x-transition">
                <v-hover v-model="hover">
                  <v-icon color="success lighten-1" large>
                    help_outline
                  </v-icon>
                </v-hover>
              </v-badge>
          ...
          <v-data-table :headers="headersRecord" :items="registro" :search="search"></v-data-table>
        </v-card>
      </template>
    </div>
    <v-footer padless>
